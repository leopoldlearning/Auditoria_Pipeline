...
- **APIs externas**: `0_2_raw_to_stage` requiere conectividad a API
...
## 7. Análisis de Tablas Vacías y Orquestación

### 7.1 Hallazgo: Tablas de Reporting sin Datos
**Problema**: Inicialmente, las tablas `fact_operaciones_mensuales`, `fact_operaciones_horarias` y `tbl_limites_pozo` estaban vacías.

**Causas Raíz**:
1. **Desfase Temporal (Notebook 1.1)**: La orquestación mensual tenía fechas fijas en `2025-01-01`.
2. **Flag Desactivado (Horaria)**: El procedimiento `sp_load_to_reporting` tiene el flag `p_procesar_horario` en `FALSE` por defecto en los notebooks de auditoría.
3. **Seed Incompleto (Límites)**: El script `V3__referencial_seed_data.sql` no poblaba la tabla de límites.

### 7.2 Hallazgo: Orquestación DQ Inexistente
**Problema**: La tabla `stage.tbl_pozo_scada_dq` está vacía a pesar de existir datos y reglas definidas.
**Causa**: No se encontró un motor de validación (script Python o SQL) que aplique las reglas de `referencial.tbl_dq_rules` al esquema V4. El sistema depende de un componente externo o no implementado para la calidad de datos.

### 7.3 Generación del Orquestador Maestro (Golden Flow)
Se ha entregado el archivo `MASTER_PIPELINE_RUNNER.py` que unifica las 7 fases del pipeline:
1.  **Re-inizialización Atómica**: Usa `DROP SCHEMA CASCADE` para garantizar un entorno limpio.
2.  **Integración de Datos Reales**: Implementa un hub para `/API Hydrog manual/`.
3.  **Ejecución de DQ**: El motor de calidad se ejecuta automáticamente como paso 5, marcando datos como `PASS`/`FAIL`.
4.  **Reporting Final**: Genera visualizaciones validadas bajo la arquitectura Zero-Calc.

### 7.4 Resultados de la Auditoría
| Hallazgo | Impacto | Resolución |
| :--- | :--- | :--- |
| Tabla DQ Vacía | Crítico | Re-mapeo de reglas a variables de telemetría y ejecución automática. |
| Ingesta Desconectada | Alto | Creación de `MASTER_PIPELINE_RUNNER.py` para sincronización. |
| Errores de Encoding | Medio | Estandarización a latin-1/UTF-8 en los cargadores de Python. |

**Estado Final: AUDITORÍA APROBADA - PIPELINE ORQUESTADO**
