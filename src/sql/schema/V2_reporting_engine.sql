-- =========================================================================
-- V2__reporting_engine.sql
-- ETL: STAGE → REPORTING (Histórico Horario, Diario, Mensual)
-- Esquema objetivo: reporting V3 (dimensiones + facts avanzadas)
-- =========================================================================
-- ============================================================
 -- 2. PROCEDIMIENTO PRINCIPAL DE REPORTING HISTÓRICO
  -- ============================================================
...
BEGIN
    ...
    INSERT INTO reporting.dim_pozo (
        ...
        nombre_yacimiento
        -- El resto de campos de dim_pozo quedan NULL o se poblarán en procesos específicos
    )
    SELECT
        m.well_id,
        m.nombre_pozo,
    ...
    ON CONFLICT (pozo_id) DO UPDATE SET
        ...
        longitud_carrera_nominal_unidad = EXCLUDED.longitud_carrera_nominal_unidad,
    ...
    IF p_procesar_horario THEN
        ...
        WITH base_horaria AS (
            SELECT 
                ...
                AVG(p.pip)                        AS pip,
                AVG(p.temperatura_tanque_aceite)  AS temp_motor,   -- proxy de temp motor
        ...
        INSERT INTO reporting.fact_operaciones_horarias (
            ...
            numero_fallas_hora
            -- El resto de columnas avanzadas quedan NULL si no se calculan aquí
        )
        SELECT
    ...
    IF p_procesar_diario THEN
        ...
        kpis_calculados AS (
            SELECT
                ...
                pd.longitud_carrera_nominal_unidad,
...
-- ============================================================
-- 3. PROCEDIMIENTO DE KPIs DE NEGOCIO (DIARIO + MENSUAL) -- 
---============================================================---
