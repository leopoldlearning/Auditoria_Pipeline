...
BEGIN
    ...
    LOOP
        ...
        IF NOT v_column_exists THEN
            -- RAISE NOTICE 'Saltando regla %: Columna % no existe o no es numérica.', v_rule.regla_id, v_rule.nombre_tecnico;
            CONTINUE;
        END IF;
        ...
        v_sql_check := format(
            'INSERT INTO stage.tbl_pozo_scada_dq (
             ...
             SELECT 
                ...
                %L::DECIMAL, 
                CASE 
                    WHEN %I::DECIMAL < %L::DECIMAL OR %I::DECIMAL > %L::DECIMAL THEN %L 
                    ELSE %L 
                END
