-- =========================================================================
-- ETL: STAGE → REPORTING - CÁLCULO DE KPIs DIARIOS (PARAMETRIZADO)
...
-- USO:
-- Ejecutar directamente (procesa CURRENT_DATE por defecto), o
-- Llamar desde procedimiento almacenado con parámetros de fecha
...
-- =========================================================================
-- Por defecto, procesa CURRENT_DATE si no se especifican parámetros
-- Se puede sobrescribir en un DO block o procedimiento almacenado

DO $$
DECLARE
    -- Parámetros configurables (NULL = usar CURRENT_DATE)
...
BEGIN
    ...
    -- Poblar dim_tiempo para el rango necesario
    PERFORM reporting.poblar_dim_tiempo(
        (v_fecha_inicio - INTERVAL '1 years')::DATE,
        (v_fecha_fin + INTERVAL '1 years')::DATE
    );
